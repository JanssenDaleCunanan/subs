...
stripe.api_key = 'sk_test_51NxQgpCBDQdVpYVX57z3HqScrgvOQFqYTlL8TypRha72QhX8JYLkApvoHdmWaNuYU6WWSkM9ictczpr2W3ZKcazu00ZkNSBJPH'
...
def updateaccounts(request):
    ...
    for customer in customers:
        print("NOTE___customer.stripe_subscription_id: " + customer.stripe_subscription_id)
        
    for customer in customers:
        print("___init for loop start____")
        try:
            subscription = stripe.Subscription.retrieve(customer.stripe_subscription_id)
        except Exception as e:
            subscription = type('', (), {'status': 'inactive'})()
        ...
        print("NOTE__ customer.stripe_subscription_id: ", customer.stripe_subscription_id, " customer.membership: ", customer.membership, "customer.cancel_at_period_end: ", customer.cancel_at_period_end)
        customer.save()
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        plan = 'price_1NxQq6CBDQdVpYVXvyWvpqPB'
        if request.POST['plan'] == 'yearly':
            plan = 'price_1NxQtrCBDQdVpYVXJcM5UNWi'
        ...
        print(customer.stripe_subscription_id)
        customer.save()
